...
using WorldEditor.Core.Diagnostics;
            ...
            if (NativePointer != IntPtr.Zero)
            {
                ...
                if (!disposing && Configuration.EnableTrackingReleaseOnFinalizer)
                {
                    if (!Configuration.EnableReleaseOnFinalizer)
                    {
                        ...
                        LogMemoryLeakWarning?.Invoke(string.Format(
                            "Warning: Live ComObject [0x{0:X}], potential memory leak: {1}", NativePointer.ToInt64(),
                            objectReference));
                    }
                }
